...
$(document).ready(function() {
    dmobscd = $("#dmobscd").text();
    setDefaultDates();
    fetchAndDisplayData(searchType, dmobscd, startD, endD);
...
function fetchAndDisplayData(searchType, rfobscd, startD, endD) {
    var apiUrl = " http://api.hrfco.go.kr/8794F4D7-ED45-43EB-8E2F-1B721623F802/dam/list/" + searchType + "/" + dmobscd + "/" + startD + "0000" + "/" + endD + ".json";
    console.log(apiUrl);
    ...
    $.get(apiUrl, function(result) {
        ...
        $.each(result.content, function(index, value) {
            ...
            table += "<td>" + (value.swl === " " ? '-' : value.swl) + "</td>";
            table += "<td>" + (value.inf === " " ? '-' : value.inf) + "</td>";
            table += "<td>" + (value.tototf === " " ? '-' : value.tototf) + "</td>";
            swl.unshift(parseFloat(value.swl));
            table += "</tr>";
