...
$(document).ready(function() {
    r_code = $("#r_code").text(); // r_code 가져오기
    setDefaultDates();
    fetchAndDisplayData(searchType, r_code, startD, endD);
...
function fetchAndDisplayData(searchType, r_code, startD, endD) {
    var apiUrl = "http://api.hrfco.go.kr/8794F4D7-ED45-43EB-8E2F-1B721623F802/waterlevel/list/" + searchType + "/" + r_code + "/" + startD + "0000" + "/" + endD + ".json";
    console.log(apiUrl);
    ...
    $.get(apiUrl, function(result) {
        ...
        $.each(result.content, function(index, value) {
            ...
            table += "<td>" + (value.wl === " " ? '-' : value.wl) + "</td>";
            waterLevel.unshift(parseFloat(value.wl));
            //20230811로 자르기 searchTime.push((value.ymdhm).slice(0,8));
            if (minyn != 'Y') {
                if (value.fw === " "|| value.fw == '-9999'){
                    table += "<td class='td_fw'>-</td>";
                } else{
                    table += "<td class ='td_fw'>" + value.fw + "</td>";
                    waterFlowRate.unshift(parseFloat(value.fw));
                }
            }
            table += "</tr>";
        ...
        function getPointFormat(searchType) {
            var commonFormat = '<br><span style="color:{point.color}">\u25CF</span> {series.name}: <b>{point.y}</b>';
            if (searchType === '10M') {
